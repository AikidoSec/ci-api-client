  ...
  "node_modules/readable-stream/lib/internal/streams/pipeline.js"(exports2, module2) {
    ...
    function destroyer(stream4, reading, writing, callback) {
      ...
      return function(err) {
        ...
        if (destroyed) return;
        destroyed = true;
        if (isRequest(stream4)) return stream4.abort();
...
var getConfigStore = () => {
  return new Configstore(program2.name());
};
var getApiKey = () => {
  return program2.getOptionValue("apikey") ? program2.getOptionValue("apikey") : getConfigStore().get("apikey");
};
var setApiKey = (newApiKey) => {
...
// lib/output.js
var outputLog = (message) => {
  if (!process.env.QUIET) {
    console.log(message);
  }
};
var outputDebug = (variable) => {
  if (process.env.CLI_DEBUG == true || process.env.CLI_DEBUG == "true") {
    console.log(source_default.blue(typeof variable == "object" ? JSON.stringify(variable) : variable));
  }
};
var outputError = (message, exitCode = 1) => {
  program2.error(message, {
    exitCode
  });
};
var outputHttpError = (axiosError) => {
...
// lib/commands/apiKey.js
var API_KEY_REGEX = /^(?:AIK_CI_|AIK_CI_US_|AIK_CI_ME_)[A-Za-z0-9]{64}$/;
function cli(apiKey) {
  ...
  setApiKey(apiKey);
  outputLog(apiKey ? "Your new Aikido API key has been set" : "Your Aikido API key has been removed");
}
var cliSetup = (program3) => program3.command("apikey").addArgument(new Argument("<key>", "Your Aikido public CI API key. See also: https://app.aikido.dev/settings/integrations/continuous-integration. To remove your apikey, leave the <key> argument value empty.").argOptional()).description("Set (or remove) your Aikido public CI API key.").action(cli);
var apiKey_default = { cli, cliSetup };
...
async function startScan(data) {
  ...
  const resultData = (await axios_default(requestUrl, requestConfig)).data;
...
var TUploadPayloadType;
(function(TUploadPayloadType2) {
  TUploadPayloadType2["Checkov"] = "checkov";
  TUploadPayloadType2["JsonSbom"] = "json-sbom";
})(TUploadPayloadType || (TUploadPayloadType = {}));
async function uploadCustomScanResult(data) {
...
async function cli2(repoId, baseCommitId, headCommitId, branchName, options, command) {
  const apiKey = getApiKey();
  if (!apiKey) {
  ...
  const onScanComplete = (pollResult) => {
    if (pollResult.gate_passed === true) {
      ...
      if (pollResult.diff_url) {
        outputLog(source_default.gray(`* Diff url: ${pollResult.diff_url}`));
      }
    } else {
      loader?.fail("Scan completed with issues");
      if (pollResult.open_issues_found) {
        outputLog(source_default.gray(source_default.bold("Open issues found: ") + pollResult.open_issues_found));
      }
      if (pollResult.issue_links) {
        outputLog(source_default.gray(pollResult.issue_links.map((issueLink) => "- " + issueLink).join("\n")));
      }
      if (pollResult.diff_url) {
        outputLog(source_default.gray(`* Diff url: ${pollResult.diff_url}`));
      }
  ...
  const onFail = (error) => {
    ...
    if (error.response?.status && error.response?.status === 404) {
      outputError("Please verify your repoId, baseCommitId, headCommitId and branchName");
    } else {
...
var scan = async ({ repoId, baseCommitId, headCommitId, branchName, options, pollInterval = 5, onStart, onStartComplete, onStartFail, onScanStart, onNextPoll, onScanComplete, onScanFail }) => {
  onStart?.();
  let result = null;
  try {
  ...
  const pollStatus = async () => {
    try {
      ...
      if (pollResult.all_scans_completed === false) {
        ...
        setTimeout(pollStatus, pollInterval * 1e3);
      } else {
...
var parseCliOptions = (userCliOptions) => {
  const apiOptions = { version: "1.0.12" };
  ...
  if (userCliOptions.pollInterval && (isNaN(userCliOptions.pollInterval) || userCliOptions.pollInterval <= 0)) {
  ...
  const onScanComplete = (pollResult) => {
    if (pollResult.gate_passed === true) {
      ...
      if (pollResult.new_issues_found) {
        outputLog(source_default.gray(source_default.bold("Open issues found: ") + pollResult.new_issues_found));
      }
      if (pollResult.issue_links) {
        outputLog(source_default.gray(pollResult.issue_links.map((issueLink) => "- " + issueLink).join("\n")));
      }
...
var scan2 = async ({ repoId, commitId, options, pollInterval = 10, onStart, onStartComplete, onStartFail, onScanStart, onNextPoll, onScanComplete, onScanFail }) => {
  onStart?.();
  let result = null;
  try {
    result = await startScan({
...
var parseCliOptions2 = (userCliOptions) => {
  const apiOptions = { version: "1.0.12" };
...
var cliSetup3 = (program3) => program3.command("scan-release").addArgument(new Argument("<repository_id>", "The internal GitHub/Gitlab/Bitbucket/.. repository id you want to scan.").argRequired()).addArgument(new Argument("<commit_id>", "The commit you want to scan").argRequired().argParser(validateCommitId2)).addOption(new Option("--no-fail-on-dependency-scan", "Don't fail when scanning depedencies...")).option("--fail-on-sast-scan", "Let Aikido fail when new static code analysis issues have been detected...").option("--fail-on-iac-scan", "Let Aikido fail when new infrastructure as code issues have been detected...").option("--fail-on-secrets-scan", "Let Aikido fail when new exposed secrets have been detected...").addOption(new Option("--minimum-severity-level <level>", "Set the minimum severity level. Accepted options are: LOW, MEDIUM, HIGH and CRITICAL.").choices(["LOW", "MEDIUM", "HIGH", "CRITICAL"])).addOption(new Option("--poll-interval [interval]", "The poll interval when checking for an updated scan result").preset(10).argParser(parseFloat)).addOption(new Option("--base-branch <branchname>", "Base branch for the release gated scan.")).description("Run a release scan of an Aikido repo.").action(cli3);
var scanRelease_default = { cli: cli3, cliSetup: cliSetup3, scan: scan2 };

// lib/commands/upload.js
var import_fs = require("fs");
var import_util4 = require("util");
async function cli4(options, command) {
  const apiKey = getApiKey();
  if (!apiKey) {
    outputError("Please set an api key using: aikido-cli-client apikey <key>");
  }
  const { apiOptions, cliOptions } = await parseCliOptions3(options);
...
var upload = async ({ data, onUploadComplete, onUploadFail }) => {
  try {
...
var parseCliOptions3 = async (userCliOptions) => {
  const apiOptions = {
    repository_id: userCliOptions.repositoryId,
    container_image_name: userCliOptions.containerImage ?? void 0,
  ...
  if (userCliOptions.type == TUploadPayloadType.JsonSbom) {
    if (userCliOptions.containerImage === void 0 || userCliOptions.containerImage.trim() === "") {
      outputError("Please provide a valid container image name with --container-image");
    }
  }
  if (userCliOptions.file) {
    try {
      const data = await (0, import_util4.promisify)(import_fs.readFile)(userCliOptions.file, "utf8");
...
var cliSetup4 = (program3) => program3.command("upload").option("-s, --scan-id <scanid>", "Set the scan id").requiredOption("-r, --repository-id <repositoryid>", "Set the repository id").option("-i, --container-image <container-image-name>", "Set the container image name. ").addOption(requiredOption(new Option("-t, --type <type>", "Set the provided payload type. Available options are: checkov, json-sbom").choices(["checkov", "json-sbom"]))).option("-f, --file <payloadFile>", "Specify the payload file. The cli command wil read the file and use the contents as payload data.").option("-p, --payload <payload>", "Set the payload").description("Upload a custom scan result.").action(cli4);
var upload_default = { cli: cli4, cliSetup: cliSetup4 };

// lib/index.js
import_dotenv.default.config(process.env.NODE_ENV ? { path: `.env.${process.env.NODE_ENV}` } : {});
var program2 = new Command();
program2.name("Aikido API Client").description("CLI api client to easily integrate the Aikido public CI API into custom deploy scripts").version("1.0.12");
apiKey_default.cliSetup(program2);
scan_default.cliSetup(program2);
scanRelease_default.cliSetup(program2);
upload_default.cliSetup(program2);
program2.option("-q, --quiet", "Disable console output when executing commands").on("option:quiet", function() {
  process.env.QUIET = true;
});
program2.option("--debug", "Add additional debug information to command output").on("option:debug", function() {
  process.env.CLI_DEBUG = true;
});
program2.option("--plain-output", "Disables the formatting on output").on("option:plain-output", function() {
  source_default.level = 0;
});
program2.option("--apikey <apikey>", "Use a cli apikey instead of reading the key from the configuration");
