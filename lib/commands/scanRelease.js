...
async function cli(repoId, headCommitId, options, command) {
    ...
    const onScanComplete = (pollResult) => {
        if (pollResult.gate_passed === true) {
            ...
            if (pollResult.open_issues_found) {
                outputLog(chalk.gray(chalk.bold('New issues found: ') + pollResult.open_issues_found));
            }
            else if (pollResult.new_issues_found) {
                outputLog(chalk.gray(chalk.bold('New issues found: ') + pollResult.new_issues_found));
            }
...
export const scan = async ({ repoId, headCommitId, options, pollInterval = 5, onStart, onStartComplete, onStartFail, onScanStart, onNextPoll, onScanComplete, onScanFail, }) => {
    onStart?.();
    let result = null;
    try {
        console.log(options);
...
export const cliSetup = (program) => program
    ...
    .addArgument(new Argument('<head_commit_id>', 'The commit you want to scan')
    .argRequired()
