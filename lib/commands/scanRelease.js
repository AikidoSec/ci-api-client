    ...
    const onScanComplete = (pollResult) => {
        ...
        if (pollResult.gate_passed === true) {
            ...
            if (apiOptions.sla_mode) {
                outputLog(chalk.gray(chalk.bold('Issues outside of SLA found: ') + pollResult.issues_outside_sla));
            }
...
const parseCliOptions = (userCliOptions) => {
    ...
    const cliOptions = { pollInterval: 10, json: false };
...
export const cliSetup = (program) => program
    ...
    .option('--sla-mode', 'Let Aikido only fail only on open issues that have gone out of SLA.')
