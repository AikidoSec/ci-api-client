...
async function cli(repoId, baseCommitId, headCommitId, branchName, options, command) {
    ...
    const onScanComplete = (pollResult) => {
        if (pollResult.gate_passed === true) {
            ...
            if (pollResult.open_issues_found) {
                outputLog(chalk.gray(chalk.bold('Open issues found: ') + pollResult.open_issues_found));
            }
    ...
    const onFail = (error) => {
        ...
        if (error.response?.status && error.response?.status === 404) {
            outputError('Please verify your repoId, baseCommitId, headCommitId and branchName. Did you add your repo to Aikido?');
        }
        else {
...
export const scan = async ({ repoId, baseCommitId, headCommitId, branchName, options, pollInterval = 5, timeout = 180, onStart, onStartComplete, onStartFail, onScanStart, onNextPoll, onScanComplete, onScanTimeout, onScanFail, }) => {
    onStart?.();
    let result = null;
    try {
...
const parseCliOptions = (userCliOptions) => {
    ...
    const cliOptions = { pollInterval: 5, timeout: 300 };
...
export const cliSetup = (program) => program
    ...
    .addArgument(new Argument('<base_commit_id>', 'The base commit of the code you want to scan (e.g. the commit where you branched from for your PR or the initial commit of your repo)')
    .argRequired()
    .argParser(validateCommitId))
    .addArgument(new Argument('<head_commit_id>', 'The latest commit you want to include in your scan (e.g. the latest commit id of your pull request)')
    .argRequired()
    .argParser(validateCommitId))
    .addArgument(new Argument('<branch_name>', 'The branch name')
    .argOptional()
    .default('main'))
    .option('--pull-request-title <title>', 'Your pull request title')
    .option('--pull-request-url <url>', 'Your pull request URL')
    .addOption(new Option('--self-managed-scanners <scanners...>', 'Set the minimum severity level. Accepted options are: LOW, MEDIUM, HIGH and CRITICAL.').choices(['checkov', 'json-sbom']))
    .option('--expected-amount-json-sboms <amount>', 'The expected amount of json sbombs')
    ...
    .addOption(new Option('--timeout [interval]', 'Define the timeout after which the client should stop polling if no result was returned.')
    .preset(300)
    .argParser(parseFloat))
