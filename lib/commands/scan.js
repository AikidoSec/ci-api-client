...
export const scan = async ({ repoId, baseCommitId, headCommitId, branchName, options = {}, pollInterval = 5, onStart = () => null, onStartComplete = startResult => null, onStartFail = error => null, onScanStart = startResult => null, onScanComplete = pollResult => null, onScanFail = error => null, }) => {
    onStart();
    let result = null;
...
export const cliSetup = (program) => program
    ...
    .addOption(new Option('--self-managed-scanners <scanners...>', 'Set the minimum severity level. Accepted options are: LOW, MEDIUM, HIGH and CRITICAL.').choices(['checkov', 'json-sbomb']))
    .option('--expected-amount-json-sbombs <amount>', 'The expected amount of json sbombs')
