...
import { outputError, outputHttpError, outputJson, outputLog, startSpinner, } from '../output.js';
...
export const scan = async ({ repoId, baseCommitId, headCommitId, branchName, options, pollInterval = 5, onStart, onStartComplete, onStartFail, onScanStart, onNextPoll, onScanComplete, onScanFail, }) => {
    onStart?.();
    let result = null;
    try {
...
const parseCliOptions = (userCliOptions) => {
    const apiOptions = { version: '1.0.16' };
    const cliOptions = { pollInterval: 5, json: false };
...
export const cliSetup = (program) => program
    ...
    .option('--json', 'Output the scan result as JSON (gate_passed and issue_breakdown)')
